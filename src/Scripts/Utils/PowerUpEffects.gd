...
var active_powerups: Dictionary = {
	PowerUpsManager.PowerUpType.LUGGAGE_AUTOMATA: false,
	PowerUpsManager.PowerUpType.TIME_SLOW: false,
	PowerUpsManager.PowerUpType.POINT_FRENZY: false,
	PowerUpsManager.PowerUpType.LUGGAGE_FREE_FOR_ALL: false
}

func activate_powerup(powerup_type: PowerUpsManager.PowerUpType):
	active_powerups[powerup_type] = true
	match powerup_type:
		PowerUpsManager.PowerUpType.LUGGAGE_AUTOMATA:
			print("Luggage Automata activated")
		PowerUpsManager.PowerUpType.TIME_SLOW:
			emit_signal("time_slow_activated", time_slow_factor)
			print("Time Slow activated")
		PowerUpsManager.PowerUpType.POINT_FRENZY:
			emit_signal("point_frenzy_activated", point_frenzy_multiplier)
			print("Point Frenzy activated")
		PowerUpsManager.PowerUpType.LUGGAGE_FREE_FOR_ALL:
			emit_signal("luggage_free_for_all_activated")
			print("Luggage Free-For-All activated")

func deactivate_powerup(powerup_type: PowerUpsManager.PowerUpType):
...
func _physics_process(delta):
	if active_powerups[PowerUpsManager.PowerUpType.LUGGAGE_AUTOMATA]:
	...
	for player in players:
		for luggage in luggage_items:
			if luggage.global_position.distance_to(player.global_position) <= luggage_automata_radius:
				if active_powerups[PowerUpsManager.PowerUpType.LUGGAGE_FREE_FOR_ALL] or luggage.luggage_type == player.character_type:
					# Collect the luggage
